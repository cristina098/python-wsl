...
print(f"You have {filesNo} Open-Meteo files")

############################################################################ read first file to build df structure
f = open(f'/home/cristina/python-wsl/python-wsl/METEO/Open-Meteo-{firstDate}.json',)
data = json.load(f)
f.close()
...
for i in range(len(data['OPENMETEO'])):
    prognosisTime = datetime.datetime.strptime(data['OPENMETEO'][i]['now'], '%Y-%m-%dT%H:%M:%S')
    prognosisTemp = data['OPENMETEO'][i]['temp']
...
for i in range(filesNo + len(data['OPENMETEO'])):
...
for i in range(filesNo):
    ...
    f = open(f'/home/cristina/python-wsl/python-wsl/METEO/Open-Meteo-{fileTime}.json',)
    ...
    for i in range(len(data['OPENMETEO'])):
        ...
        deltaTime = prognosisTime - nowTime
        diferenta = deltaTime.days*24 + deltaTime.seconds//3600
        allData.at[data['OPENMETEO'][i]['now'], diferenta] = prognosisTemp
...
#allData.to_csv('/home/cristina/python-wsl/python-wsl/METEO/Open-Meteo.csv')
#allDataDiff.to_csv('/home/cristina/python-wsl/python-wsl/METEO/Open-Meteo-diff.csv')
