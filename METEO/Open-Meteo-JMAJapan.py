...
url = "https://api.open-meteo.com/v1/jma?latitude=47.151726&longitude=27.587914&hourly=temperature_2m&timezone=Africa%2FCairo"
req = requests.get(url, headers=headers)
...
for i in range(len(OpenMeteoTimes)):
    ...
    if (prognosisTime.hour in [3, 9, 15, 21]) and (deltaTime.total_seconds() > -2000):
        ...
        myDict["JMAJapan"].append({"now": f"{prognosisTime.isoformat()}", "temp": OpenMeteoTemps[i]})

filename = f"JMA-Japan-{myDict['RO']['now']}.json"
